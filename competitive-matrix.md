...
| **Coinbase Agentic Wallets** | Yes — Coinbase MPC custody | Partial (CDP SDK is OSS; custody infra is not) | No — keys on Coinbase infrastructure | x402 native (theirs); MPP and AP2 not native | Coinbase reporting; not a user-walkable chain | Public company (Coinbase, NASDAQ: COIN) | Centralized custodian; single-vendor risk; KYC/regulatory exposure on the funds themselves |

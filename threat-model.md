...
- Issuance: [`quaestor-core/src/lib/mandate.ts`](https://github.com/Kabukich0/quaestor-core/blob/main/src/lib/mandate.ts) — every mandate JWT carries `amount_max, currency, network, purpose_tag, use_counter_max, exp, nbf, jti`.
- Redemption check: [`quaestor-core/src/lib/server.ts`](https://github.com/Kabukich0/quaestor-core/blob/main/src/lib/server.ts) `POST /mandate/redeem` rejects with `409 amount_exceeds_max` and `409 use_counter_exhausted` (proven by `quaestor-core/test/mandate.test.ts`).
...
- Vault encryption: [`quaestor-core/src/lib/vault.ts`](https://github.com/Kabukich0/quaestor-core/blob/main/src/lib/vault.ts).
- KEK in OS keychain: [`quaestor-core/src/lib/auth.ts`](https://github.com/Kabukich0/quaestor-core/blob/main/src/lib/auth.ts) (writes/reads via `keytar`).
- Bridge has zero key material: [`quaestor-bridge/test/ap2.test.ts → "no key material leaks"`](https://github.com/Kabukich0/quaestor-bridge/blob/main/test/ap2.test.ts) greps every `.ts`/`.js` for private-key-shaped strings every CI run.
...
- Append + chain construction: [`quaestor-core/src/lib/ledger.ts`](https://github.com/Kabukich0/quaestor-core/blob/main/src/lib/ledger.ts) (`entry_hash = BLAKE3(ts | type | jti | payload | prev_hash)`).
- Verification CLI: [`quaestor-core/src/commands/ledger/verify.ts`](https://github.com/Kabukich0/quaestor-core/blob/main/src/commands/ledger/verify.ts).
- Tampering tests: [`quaestor-core/test/ledger.test.ts`](https://github.com/Kabukich0/quaestor-core/blob/main/test/ledger.test.ts) covers payload edits and `prev_hash` rewrites — both are detected at the broken row.
...
- The daemon never holds value: [`quaestor-core/src/lib/server.ts`](https://github.com/Kabukich0/quaestor-core/blob/main/src/lib/server.ts) only signs JWTs and emits credential headers; balance state lives entirely with the upstream facilitator.
- Bridge does not custody either: [`quaestor-bridge/src/adapters/`](https://github.com/Kabukich0/quaestor-bridge/tree/main/src/adapters) all three adapters return wire-format headers, never wallet balances.
...
- Loopback enforcement: [`quaestor-core/src/lib/server.ts`](https://github.com/Kabukich0/quaestor-core/blob/main/src/lib/server.ts).
- Bridge → core path: [`quaestor-bridge/src/core-client.ts`](https://github.com/Kabukich0/quaestor-bridge/blob/main/src/core-client.ts) — base URL is loopback; only overridable in tests.
- Mainnet guard at the boundary: [`quaestor-bridge/src/network.ts`](https://github.com/Kabukich0/quaestor-bridge/blob/main/src/network.ts) `assertMainnetAllowed()` is invoked by every adapter constructor.
...
- Auth token write + permissions: [`quaestor-core/src/commands/init.ts`](https://github.com/Kabukich0/quaestor-core/blob/main/src/commands/init.ts) writes the token at startup; [`quaestor-core/src/lib/server.ts`](https://github.com/Kabukich0/quaestor-core/blob/main/src/lib/server.ts) compares with `crypto.timingSafeEqual` on every request.
- Bridge re-resolves the token from the same canonical path: [`quaestor-bridge/src/core-client.ts → resolveAuthTokenPath`](https://github.com/Kabukich0/quaestor-bridge/blob/main/src/core-client.ts).
